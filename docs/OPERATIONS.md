...
First time on a Hive, and again **after every T-Pot update** (which can replace T-Pot's `docker-compose.yml` and drop the nginx mounts):

```bash
./tpot-expansion-pack.sh         # .env + secrets (prompts), T-Pot nginx routes + landing page, starts the stack
./tpot-expansion-pack.sh --setup # replace the DigitalOcean token, EDL_ALLOW or GCP key
```

See the README's [Quick start](../README.md#quick-start) and [Behind T-Pot's nginx](../README.md#behind-t-pots-nginx-default).
...
```bash
curl -s http://172.17.0.1:8880/api/droplets/<id>/health              # ping + TCP checks
curl -s 'http://172.17.0.1:8880/api/droplets/<id>/health?deep=true'  # also runs the health playbook
```
...
| DigitalOcean token | `secrets/do_token` (host, `600`, gitignored; the only place the app reads it from) |
| GCP service account key | `secrets/gcp-sa.json` (host, `600`, gitignored) |
| Deployer keys | `secrets/ssh_key`, `secrets/ssh_key.pub` (host, gitignored) |
| T-Pot nginx routes and landing page | `$TPOT_HOST_DIR/data/nginx/conf/tpotweb.conf` and `index.html`, generated by `tpot-expansion-pack.sh` and mounted into T-Pot's nginx |
| Firewall allowed to fetch the EDL | `EDL_ALLOW` in `.env` (applied by `tpot-expansion-pack.sh`) |
...
| Banner: *No DigitalOcean API token* | `secrets/do_token` is missing or empty. Run `./tpot-expansion-pack.sh --setup`, or write the token to that file with mode `600`. No restart needed. The token can't be set from the UI. |
| `/sensors/` returns *502 Bad Gateway* | T-Pot's nginx can't reach the deployer. Check `docker compose ps` here, then re-run `./tpot-expansion-pack.sh`, which also resets `WEB_BIND` to the Docker bridge IP. |
| `/sensors/` or the landing page's *Sensor Deployer* link is gone after a T-Pot update | The update replaced T-Pot's `docker-compose.yml`. Re-run `./tpot-expansion-pack.sh`. |
| The firewall's EDL fetches get `401` | Its address isn't in `EDL_ALLOW`. See [PALO_ALTO_EDL.md](PALO_ALTO_EDL.md#who-can-fetch-it). |
